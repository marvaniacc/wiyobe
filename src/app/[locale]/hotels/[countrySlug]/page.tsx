...
/**
 * Public SSR route — renders a card grid of verified hotels located in the
 * requested country. Hotels have NO ProviderLocation relation, so they are
 * filtered solely by their primary `country` field.
 */
      ...
      <section className="border-b border-divider bg-gradient-to-b from-primary/5 to-transparent">
        <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
          <div className="mb-3 flex items-center gap-2 text-sm text-muted-foreground">
            <Link href={`/${locale}/hotels`} className="transition-colors hover:text-primary">
              ← All hotels
            </Link>
          </div>
          ...
          <p className="mt-3 max-w-2xl text-base text-muted-foreground sm:text-lg">
            Verified hotels and accommodations for medical tourism stays in {countryName}.
          </p>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
        {hotels.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-[16px] border border-dashed border-divider bg-surface py-16 text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground">No hotels in {countryName}</h2>
            <p className="max-w-sm text-sm text-muted-foreground">
              We don&apos;t have any verified hotel profiles in {countryName} yet. Try browsing all countries.
            </p>
            <Link
              href={`/${locale}/hotels`}
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {hotels.map((hotel) => {
              const detailCountrySlug = hotel.country ? getCountrySlug(hotel.country) : countrySlug
              const href =
                hotel.slug && detailCountrySlug
                  ? `/${locale}/hotels/${detailCountrySlug}/${hotel.slug}`
                  : `/${locale}/hotels/${countrySlug}`
              return (
                <Link
                  key={hotel.id}
                  href={href}
