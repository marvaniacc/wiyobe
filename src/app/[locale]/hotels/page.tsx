...
export const metadata: Metadata = {
  title: 'Hotels — Wishubest Medical Tourism',
  description:
    'Browse verified hotels and accommodations for medical tourism worldwide. Compare star ratings, prices per night, and reviews.',
  openGraph: {
    title: 'Verified Hotels — Wishubest Medical Tourism',
    description:
      'Browse verified hotels and accommodations for medical tourism worldwide.',
    type: 'website',
  },
}
...
/**
 * Public SSR route — renders a card grid of ALL verified hotels whose
 * associated User has KYC APPROVED status.
 *
 * NOTE: Hotels do NOT have a ProviderLocation relation — each hotel is an
 * independent bookable entity and is filtered solely by its primary `country`
 * field on the country-filtered page.
 */
export default async function HotelsListPage({
  ...
  const hotels: HotelCard[] = await db.hotel.findMany({
    where: {
      verified: true,
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b border-divider bg-gradient-to-b from-primary/5 to-transparent">
        <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
          <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
            Verified Hotels &amp; Accommodations
          </h1>
          <p className="mt-3 max-w-2xl text-base text-muted-foreground sm:text-lg">
            Browse hotels and suites ideal for medical tourism stays worldwide. Compare star
            ratings, prices per night, and guest reviews.
          </p>
        </div>
      </section>
      ...
      <div className="border-b border-divider bg-surface">
        <div className="mx-auto flex max-w-5xl flex-wrap gap-2 px-4 py-3 sm:px-6">
          ...
          <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">Hotels</span>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
        {hotels.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-[16px] border border-dashed border-divider bg-surface py-16 text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground">No hotels available</h2>
            <p className="max-w-sm text-sm text-muted-foreground">
              Verified hotel profiles will appear here soon. Please check back later.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {hotels.map((hotel) => {
              const countrySlug = hotel.country ? getCountrySlug(hotel.country) : null
              const href =
                hotel.slug && countrySlug
                  ? `/${locale}/hotels/${countrySlug}/${hotel.slug}`
                  : `/${locale}/hotels`
              const flag = getCountryFlag(hotel.country || '')
              const displayCountry = hotel.country
                ? getCountryName(hotel.country.toLowerCase()) || hotel.country
                : hotel.country
              return (
                <Link
                  key={hotel.id}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="line-clamp-1 text-sm text-muted-foreground">
                        {'★'.repeat(Math.max(1, Math.min(5, hotel.starRating)))} {hotel.starRating}-star
                      </p>
                      <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">
                        <span className="me-1">{flag}</span>
                        {hotel.city}
                        {hotel.city && displayCountry ? ', ' : ''}
                        {displayCountry}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-divider pt-4">
                    ...
                    <div className="text-end">
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">From / night</p>
